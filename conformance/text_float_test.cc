...
TEST_P(TextFloatLiteralTest, Zero) {
  EXPECT_THAT(RequiredTest().ParseText(message(), Input("0")).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(""))));
  EXPECT_THAT(RequiredTest().ParseText(message(), Input("0")).SerializeText(),
              Yields(ParsedPayload(EqualsTextProto(""))));
}
...
TEST_P(TextFloatTest, NoHex) {
  EXPECT_THAT(
      RequiredTest().ParseText(message(), "optional_float: 0x1").ParseOnly(),
      Yields(IsParseError()));
}
...
TEST_P(TextFloatTest, NoOctal) {
  EXPECT_THAT(
      RequiredTest().ParseText(message(), "optional_float: 012").ParseOnly(),
      Yields(IsParseError()));
}
