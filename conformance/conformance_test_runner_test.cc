...
// Parses `args` (without argv[0], which is added) with the one legacy suite
// the real runner has left, whose failure list flag is --failure_list.  The
// parser accepts --text_format_failure_list on top of that, for the gtest
// text-format suite (see kGtestOnlyFailureListFlags in
// conformance_test_runner.cc); the tests below cover both.
...
std::string NewOutputDir() {
  ...
  std::string dir =
      absl::StrCat(testing::TempDir(), test_info.test_suite_name(), "_",
                   test_info.name(), "/");
  EXPECT_TRUE(mkdir(dir.c_str(), 0755) == 0 || errno == EEXIST) << dir;
  return dir;
}
...
TEST(RunSuiteTest, EmptyOutputDirWritesToTheCurrentDirectory) {
  ...
  ASSERT_EQ(chdir(output_dir.c_str()), 0) << output_dir;
  ...
  EXPECT_EQ(chdir(previous_dir.c_str()), 0) << previous_dir;
