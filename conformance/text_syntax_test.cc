...
// For gtest's failure messages (the parameter is otherwise printed as bytes).
void PrintTo(const SeparatorCase& test_case, std::ostream* os) {
  *os << test_case.type;
}
...
void PrintTo(FieldCardinality cardinality, std::ostream* os) {
  *os << ParamName(cardinality);
}
...
// --maximum_edition doesn't cover it.  Input() only builds values; every
// expectation is in the test bodies.
...
TEST_P(TextFieldSeparatorTest, Comma) {
  ...
  EXPECT_THAT(RequiredTest().ParseText(message(), input).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RequiredTest().ParseText(message(), input).SerializeText(),
              Yields(ParsedPayload(EqualsTextProto(input))));
}
...
TEST_P(TextFieldSeparatorTest, CommaDuplicatesFails) {
  EXPECT_THAT(RequiredTest().ParseText(message(), Input(",,")).ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextListSeparatorTest, Missing) {
  ...
  if (test_case().adjacent_values_concatenate) {
    EXPECT_THAT(RequiredTest("IsOneValue")
                    .ParseText(message(), input)
                    .SerializeBinary(),
                Yields(ParsedPayload(EqualsTextProto(input))));
    EXPECT_THAT(
        RequiredTest("IsOneValue").ParseText(message(), input).SerializeText(),
        Yields(ParsedPayload(EqualsTextProto(input))));
  } else {
    EXPECT_THAT(RequiredTest("Fails").ParseText(message(), input).ParseOnly(),
                Yields(IsParseError()));
  }
}
