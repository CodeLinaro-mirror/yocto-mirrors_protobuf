...
TEST(TesteeTest, DerivedName) {
  ...
  TestResult result =
      Checked(testee
                  .CreateTest({.suite = "FooTest", .test = "Bar"},
                              TestStrictness::kRequired)
                  .ParseBinary(TestAllTypesProto2::descriptor(), Wire("wire"))
                  .SerializeBinary());
...
TEST(TesteeTest, DerivedNameCarriesTheInputFormat) {
  ...
  TestResult text = Checked(testee
                                .CreateTest({.suite = "FooTest", .test = "Baz"},
                                            TestStrictness::kRequired)
...
TEST(TesteeDeathTest, DerivedNameNeedsASuiteAndATest) {
  MockTestRunner mock;
  Testee testee(&mock);
  // Spelled out as TestName{...}: with a single designator GCC 10 otherwise
  // considers the string_view overload viable and reports an ambiguity.
  EXPECT_DEATH(
      testee.CreateTest(TestName{.test = "Bar"}, TestStrictness::kRequired),
      "needs a gtest suite and test name");
  EXPECT_DEATH(testee.CreateTest(TestName{.suite = "FooTest"},
                                 TestStrictness::kRequired),
               "needs a gtest suite and test name");
}
...
TEST(TesteeDeathTest, DerivedNamePiecesMustBeGtestIdentifiers) {
  MockTestRunner mock;
  Testee testee(&mock);
  EXPECT_DEATH(testee.CreateTest({.suite = "All/FooTest", .test = "Bar"},
                                 TestStrictness::kRequired),
               "may only contain letters, digits and '_': All/FooTest");
  EXPECT_DEATH(testee.CreateTest({.suite = "FooTest", .test = "Bar.Baz"},
                                 TestStrictness::kRequired),
               "may only contain letters, digits and '_': Bar.Baz");
  EXPECT_DEATH(testee.CreateTest(
                   {.suite = "FooTest", .test = "Bar", .params = "INT32/1"},
                   TestStrictness::kRequired),
               "may only contain letters, digits and '_': INT32/1");
  EXPECT_DEATH(
      testee.CreateTest({.suite = "FooTest", .test = "Bar", .suffix = "*"},
                        TestStrictness::kRequired),
...
TEST(PrintTestResultTest, LargeTextInputIsTruncated) {
  ...
  EXPECT_EQ(
      ...
      absl::StrCat(
          ...
          R"(message_type: "protobuf_test_messages.proto2.TestAllTypesProto2" )"
          R"(test_category: TEXT_FORMAT_TEST text_payload: ")",
          std::string(200, 'a'), "...(truncated)",
          R"("} and response {runtime_error: "error"})"));
}
