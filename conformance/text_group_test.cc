...
TEST_P(TextGroupTest, Empty) {
  EXPECT_THAT(RequiredTest().ParseText(message(), "Data {}").SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto("Data {}"))));
  EXPECT_THAT(RequiredTest().ParseText(message(), "Data {}").SerializeText(),
              Yields(ParsedPayload(EqualsTextProto("Data {}"))));
}
...
TEST_F(TextDelimitedTest, GroupFieldEmpty) {
  EXPECT_THAT(
      RequiredTest().ParseText(message(), "GroupLikeType {}").SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto("GroupLikeType {}"))));
  EXPECT_THAT(
      RequiredTest().ParseText(message(), "GroupLikeType {}").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto("GroupLikeType {}"))));
}
