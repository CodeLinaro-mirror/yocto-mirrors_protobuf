...
// test.  The full name is
//
//   <Level>.<Suite>.<Test>[/<Params>][.<Suffix>].<Input>Input[.<Output>Output]
//
// where <Level> is the strictness, <Input>Input the format of the request's
...
class InMemoryMessage {
 public:
  ...
  // "<Level>.<Syntax>.JsonInput.<name>" (RunValidJsonTestOrParseFailure); see
  // ParseOnlyOptions.
  //
 ...
 private:
  InMemoryMessage(class Testee* testee, TestName name,
                  absl::string_view legacy_name, TestStrictness strictness,
                  const Descriptor* type,
                  ::conformance::ConformanceRequest request)
      : testee_(testee),
        name_(std::move(name)),
        legacy_name_(legacy_name),
...
class Test {
 ...
 private:
  Test(class Testee* testee, TestName name, absl::string_view legacy_name,
       TestStrictness strictness)
      : testee_(testee),
        name_(std::move(name)),
        legacy_name_(legacy_name),
...
class Testee {
 public:
  ...
  // Creates a test named after the enclosing gtest test; see TestName.
  // Check-fails if `name` has an empty suite or test, or a piece with a
  // character gtest wouldn't allow.
  Test CreateTest(TestName name, TestStrictness strictness);

  // Creates a test with an explicit name, "<Level>.<Syntax>.<Input>Input.
  // <name>[.<Output>Output]".
  // TODO: b/564135448 - remove once every suite derives its names from gtest.
  Test CreateTest(absl::string_view name, TestStrictness strictness) {
    return Test(this, TestName(), name, strictness);
  }
