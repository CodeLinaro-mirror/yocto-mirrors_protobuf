...
std::string ForkPipeRunner::GetTestProgramFailure(ReadResult read_result) {
  // The testee produced no response: it exited, crashed, or hung.  After a
  // timeout TryRead() has already terminated it (that is the only way to
  // unblock the reader thread), so there is nothing to wait for; after EOF or
  // a read error there is nothing to wait politely for either, so it is shut
  // down with no grace period (terminated at once if it is still alive).
  // Either way the outcome is classified from the exit code.
  ABSL_LOG(INFO) << "Trying to reap child";
  const ShutdownResult shutdown = Shutdown(std::chrono::milliseconds::zero());
  ...
  if (read_result == ReadResult::kTimeout) {
    ...
    // The testee was still alive after the failed read (e.g. it closed its
    // stdout but kept running) and Shutdown() terminated it.
    ...
    // No exit code to classify; all we know is the read failed.
    error_msg = read_result == ReadResult::kEof
                    ? "child closed its output without responding"
                    : "error reading from child";
  }
