...
// gtest-based suites of the runner (conformance_test_main.cc) load every
// failure list given, whatever its flag, so these keep working now that every
// suite has moved to gtest.  --failure_list was the legacy binary/JSON suite's
// flag and --text_format_failure_list the legacy text-format suite's; the test
// targets and scripts still pass both.  The files are recorded under their
// own flag name (see ConformanceRunnerOptions::failure_list_files), so that
// the runner can tell which list an entry came from.
// TODO: b/563657875 - make this the plain list of failure list flags once the
// sh_test/JSPB invocations move to the conformance_test() macro and `suites`
// goes away.
