...
// Failure list flags accepted although no legacy suite loads their files: the
// gtest-based suites of the merged runner (conformance_test_main.cc) load
// every failure list given, whatever its flag, so these keep working for
// suites that have entirely moved to gtest.  --text_format_failure_list was
// the legacy text-format suite's flag; the text-format tests now live in the
// gtest text suite, and the test targets and scripts still pass the flag.
// The files are recorded under this flag name (see
// ConformanceRunnerOptions::failure_list_files), not as --failure_list, so
// that the merged runner can tell which list an entry came from.
// TODO: b/563657875 - drop the alias once the sh_test/JSPB invocations move
// to the conformance_test() macro.
constexpr absl::string_view kGtestOnlyFailureListFlags[] = {
...
absl::StatusOr<ConformanceRunnerOptions> ParseConformanceRunnerArgs(
  ...
  for (int arg = 1; arg < argc; ++arg) {
    if (strcmp(argv[arg], "--performance") == 0) {
      ...
      for (absl::string_view flag : kGtestOnlyFailureListFlags) {
        if (recognized_flag) break;
        if (argv[arg] == flag) {
          if (arg + 1 == argc) return missing_value(arg);
          options.failure_list_files.emplace_back(flag, argv[++arg]);
          recognized_flag = true;
        }
      }
      if (!recognized_flag) {
