...
// (ConformanceRequestSetting only takes the prototype's descriptor and
// compares against prototype.New()), so nothing replaces it here.
...
TEST_P(TextMapTest, AlphabeticallySortedMapStringKeys) {
  ...
  EXPECT_THAT(RequiredTest("AlphabeticallySortedMapStringKeys")
...
TEST_P(TextMapTest, AlphabeticallySortedMapIntKeys) {
  ...
  EXPECT_THAT(RequiredTest("AlphabeticallySortedMapIntKeys")
                  .ParseText(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kInput))));
  EXPECT_THAT(RequiredTest("AlphabeticallySortedMapIntKeys")
...
TEST_P(TextMapTest, AlphabeticallySortedMapBoolKeys) {
  ...
  EXPECT_THAT(RequiredTest("AlphabeticallySortedMapBoolKeys")
                  .ParseText(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kInput))));
  EXPECT_THAT(RequiredTest("AlphabeticallySortedMapBoolKeys")
...
TEST_P(TextMapTest, DuplicateMapKey) {
  ...
  EXPECT_THAT(RequiredTest("DuplicateMapKey")
                  .ParseText(message(), kInput)
                  .SerializeBinary(),
