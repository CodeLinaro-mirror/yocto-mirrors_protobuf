...
// the proto3-style message types only; the requests sent to the testee are
// identical to the legacy ones, except that the binary-input tests below are
...
TEST_P(JsonValueTest, ListValueDeepNesting25) {
  ...
  const std::string expected = absl::StrCat("optional_value: {\n",
                                            ...
                                            Repeat("    }\n"
                                                   ...
                                                   25),
                                            "}\n");
  EXPECT_THAT(RecommendedTest().ParseJson(message(), input).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(expected))));
  EXPECT_THAT(RecommendedTest().ParseJson(message(), input).SerializeJson(),
              Yields(ParsedPayload(EqualsTextProto(expected))));
}
...
TEST_P(JsonValueTest, ListValueDeepNesting200) {
  const std::string input =
      absl::StrCat(R"({"optionalValue": )", std::string(200, '['), "1",
                   std::string(200, ']'), "}");
  EXPECT_THAT(RecommendedTest().ParseJson(message(), input).ParseOnly(),
              Yields(IsParseError()));
}
