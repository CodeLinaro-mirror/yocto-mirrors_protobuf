...
TEST(TestManagerDeathTest, ReportRecommendedFailureRejectsListedTest) {
  // A listed test must go through ReportFailure() whatever its strictness,
...
TEST_F(TestManagerTest, SaveFailureListExpandedWildcardKeepsSkippedMatch) {
  // A skipped test says nothing about its entry, so when the wildcard is
  // expanded (here because another match passed) the skipped test keeps a line
  // with the wildcard's message; dropping it is up to the user.
  CreateFailureList(R"(
foo.*.bar # abc
)");
  TestManager manager;
