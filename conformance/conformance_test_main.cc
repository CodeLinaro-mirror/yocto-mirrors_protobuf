...
// named on its command line.  The command line is still the legacy runner's
// (see PrintConformanceRunnerUsage()), so nothing changes for its users, the
// test targets and scripts that invoke it with their failure lists and
// --output_dir:
//
...
//   * --failure_list and --text_format_failure_list, the legacy binary/JSON
//     and text-format suites' flags (see kGtestOnlyFailureListFlags in
//     conformance_test_runner.cc), are loaded into the gtest suites' one
//     TestManager.  An entry that matches no test is reported as unmatched, in
//     the legacy format, and fails the run like it failed the legacy suites.
...
//   * --record_requests writes every request into one file.
...
//
// The legacy suites themselves are gone: every test lives in a gtest suite
// now.  What remains of their plumbing (ConformanceTestSuite,
// RunConformanceSuites() and the legacy-phase branches below, which run with
// no suite) is dead and goes with the sh_test wrappers.
// TODO: b/563657875 - remove the legacy plumbing once the sh_test/JSPB
// invocations move to the conformance_test() macro.
//
...
//     affected the legacy suites.  gtest's own flags take their place
...
//     that don't exist, the legacy "do not exist" report is printed first and
//     the write error last.
...
bool ReportFailureListResults(const FailureListReport& report,
  ...
  for (const auto& [test_name, matched_entry] : report.listed_skips) {
    listed_skips.push_back(
        {.test_name = test_name,
         .failure_message = absl::StrCat("skipped by the testee (matched to ",
                                         matched_entry, ")"),
...
absl::StatusOr<GtestPhaseResult> RunGtestPhase(
  ...
  // The gtest phase only exists if some gtest suite is linked in.  (Nothing
  // runs otherwise and the failure lists go unchecked; no current binary gets
  // here.)
  ...
  // --performance was given and no performance test is linked in): a full run
  // of nothing, which matched no entry and loaded no list, so there is nothing
  // to check.
...
int Main(int argc, char* argv[]) {
  // No legacy suite is left (see the file comment), so no flag's failure
  // lists are loaded by one.
  // TODO: b/563657875 - remove the legacy plumbing.
  const std::vector<ConformanceTestSuite*> suites;
  const std::vector<std::string> legacy_failure_list_flags;
  ...
  // The legacy phase, with no suite left to run: all it still does is report
  // the tests --test named that exist nowhere (the gtest suites ran the
  // others).
