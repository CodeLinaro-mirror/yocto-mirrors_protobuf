...
//
// TODO: b/563707827 - retire the legacy-compatible parts of this command line
// (--failure_list / --text_format_failure_list and conformance_test_wrapper.sh)
// once the remaining sh_test and script invocations have moved to the
// conformance_test() macro, which runs the gtest suites as ordinary test
// binaries (see test_environment_main.cc).
//
...
//     TODO: b/563707827 - goes away with the --text_format_failure_list flag;
//     conformance_test() passes one failure list per suite instead.
...
absl::StatusOr<GtestPhaseResult> RunGtestPhase(
  ...
  // for recording and for --test isolation.  The runners only need to live for
  // this call: the environment installed below outlives them, but it doesn't
  // own the runner and drops its reference to it in TearDown() (run by gtest,
  // or below if gtest selected no test); ~ConformanceEnvironment only
  // finalizes its TestManager.
