...
// which ran for the proto3-style message types only; the requests sent to the
// testee are identical to the legacy ones, except that the binary-input tests
// below are BINARY_TEST requests (see below).
//
...
// test, under "ProtobufInput.<name>.JsonOutput"; the same bytes are built here
// with the wire-format builders, and ParseBinary() categorizes them as
// BINARY_TEST, which no testee tells apart from JSON_TEST on a binary payload
// (b/563657722).
...
TEST_P(JsonFieldMaskTest, FieldMask) {
  ...
  EXPECT_THAT(RequiredTest().ParseJson(message(), kInput).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(RequiredTest().ParseJson(message(), kInput).SerializeJson(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonFieldMaskTest, FieldMaskPathsDontRoundTrip) {
  EXPECT_THAT(RecommendedTest()
                  .ParseBinary(message(), LengthPrefixedField(
                                              kOptionalFieldMask,
                                              LengthPrefixedField(
                                                  kFieldMaskPaths, "fooBar")))
                  .SerializeJson(),
