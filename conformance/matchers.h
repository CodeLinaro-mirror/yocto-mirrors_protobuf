...
// conformance runner for every case the legacy runner covered.  Cases it
// didn't cover produce new text: an inner matcher without an explanation of
...
// actual bytes).
//
// Like the legacy runner's byte-exact mode, a PROTOBUF payload is first
// checked to be parseable as the test's message type (failing with
