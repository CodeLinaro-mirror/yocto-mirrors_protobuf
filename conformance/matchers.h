...
// `Yields(Not(ParsedPayload(...)))`.  ParsedPayload() and Payload() take care
// of decoding the testee's response and accept any gMock matcher for the
// decoded value.  EqualsTextProto() and EqualsBinaryProto() are ordinary
// matchers on `const Message&` that are intended to be used inside
// ParsedPayload(), but work anywhere.
//
...
//   - A failure of a kRecommended test is tolerated (logged as a WARNING and
//     counted, but the test passes) unless the TestManager enforces
//     recommended tests or the test is listed in the failure list.
...
//     counts as seen, so Finalize() doesn't report it again, and --fix drops
//     it (see TestManager::SaveFailureList()).  The TestManager records such
//     tests in ListedSkips().  This is deliberately stricter than the legacy
//     runner, which passed a listed test the testee skipped.
...
// to the output format the test requested (binary or text), matches `m`.  `m`
// may be any matcher on `const Message&`, typically EqualsTextProto() or
// EqualsBinaryProto().
//
...
// unparseable." message.  JSON output can't be decoded yet (b/410122158) and
// fails with a message saying so; use Payload() for it.
template <typename M>
...
//
//   EXPECT_THAT(RequiredTest("X").ParseBinary(d, w).SerializeBinary(),
...
// list, test strictness and skip policy described at the top of this file.
