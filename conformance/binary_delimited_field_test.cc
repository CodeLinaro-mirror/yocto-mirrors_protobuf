...
// under editions.  This holds the binary-output leg of the legacy suite-level
// BinaryAndJsonConformanceSuite::RunDelimitedFieldTests(); the legacy suite
// still sends the binary->JSON leg of the same DelimitedFieldTest cases.  The
...
TEST_F(DelimitedFieldTest, ValidNonMessage) {
  EXPECT_THAT(
      RequiredTest("ValidNonMessage")
          .ParseBinary(message(), VarintField(1, 99))
          .SerializeBinary(),
...
TEST_F(DelimitedFieldTest, ValidMapLengthPrefixed) {
  EXPECT_THAT(
      RequiredTest("ValidMap.LengthPrefixed")
...
TEST_F(DelimitedFieldTest, GroupLike) {
  EXPECT_THAT(
      RequiredTest("ValidDelimitedField.GroupLike")
...
TEST_F(DelimitedFieldTest, NotGroupLike) {
  EXPECT_THAT(
      RequiredTest("ValidDelimitedField.NotGroupLike")
...
TEST_F(DelimitedExtensionTest, GroupLike) {
  EXPECT_THAT(
      RequiredTest("ValidDelimitedExtension.GroupLike")
...
TEST_F(DelimitedExtensionTest, NotGroupLike) {
  EXPECT_THAT(
      RequiredTest("ValidDelimitedExtension.NotGroupLike")
