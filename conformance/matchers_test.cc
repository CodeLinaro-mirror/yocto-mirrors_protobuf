...
TEST(RawPayloadTest, UnparseableProtobufOutputFailsWhateverTheMatcher) {
  // Like the legacy runner's byte-exact mode, binary output must decode as
  // the test's message type.
  TestResult result = CreateResult("foo", ::conformance::PROTOBUF,
                                   ProtobufPayload(Wire("\001")));
...
TEST(FailureMatcherTest, ParseErrorRuntimeErrorWithBinaryInput) {
  // The legacy binary runner reported runtime errors specially.
  TestResult result =
...
TEST(FailureMatcherTest, ParseErrorRuntimeErrorWithTextInput) {
  // The legacy text format runner didn't distinguish runtime errors.
  TestResult result =
...
TEST(FailureMatcherTest, ParseErrorRuntimeErrorWithJsonInput) {
  // Neither did the legacy JSON runner.
  TestResult result =
...
constexpr absl::string_view kRequiredFoo =
    "Required.YieldsTest.foo.ProtobufInput.ProtobufOutput";
constexpr absl::string_view kRecommendedFoo =
    "Recommended.YieldsTest.foo.ProtobufInput.ProtobufOutput";
...
// The TestName of the test `name` of these tests' suite.
internal::TestName Named(absl::string_view name) {
  return {.suite = "YieldsTest", .test = std::string(name)};
}
...
class YieldsTest : public testing::Test {
 protected:
  ...
  TestResult Run(absl::string_view response,
    ...
    return testee_.CreateTest(Named(name), strictness)
...
TEST_F(YieldsTest, TextFormatOutput) {
  ...
  EXPECT_THAT(
      ...
      ElementsAre(
          FieldsAre(
              "Required.YieldsTest.mismatch.ProtobufInput.TextFormatOutput",
              "Output was not equivalent to reference message: "
              "modified: optional_int32: 1 -> 2",
              absl::nullopt),
          FieldsAre(
              "Required.YieldsTest.unparseable.ProtobufInput.TextFormatOutput",
              "TEXT_FORMAT output we received from test was unparseable.",
              absl::nullopt)));
}
...
TEST_F(YieldsTest, ListedFailureViaWildcardPasses) {
  ASSERT_THAT(test_manager_.AddExpectedFailure(
                  "Required.YieldsTest.foo.*.ProtobufOutput", kNotAParseError),
              IsOk());
...
TEST_F(YieldsTest, ListedSuccessViaWildcardNamesTheWildcard) {
  ASSERT_THAT(test_manager_.AddExpectedFailure(
                  "Required.YieldsTest.foo.*.ProtobufOutput", ""),
              IsOk());

  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(Run(kParseError), Yields(IsParseError())),
      "test Required.YieldsTest.foo.ProtobufInput.ProtobufOutput (matched to "
      "Required.YieldsTest.foo.*.ProtobufOutput) is in the failure list, "
...
TEST_F(YieldsTest, ListedSkipFailsOnce) {
  ...
  // seen, so Finalize() doesn't report it a second time, and --fix drops it
  // like that of a test that passed.
...
TEST_F(YieldsTest, NotComposes) {
  ...
  EXPECT_THAT(Explain(matcher, result),
              Rejects(absl::StrCat(
                  ...
                  "Required.YieldsTest.second.ProtobufInput.ProtobufOutput")));
  EXPECT_NONFATAL_FAILURE(EXPECT_THAT(result, matcher),
                          "which doesn't match (is not a parse error)");
  EXPECT_EQ(GetCounts(test_manager_),
            (Counts{.expected_successes = 1, .unexpected_failures = 1}));
...
TEST_F(YieldsTest, CheckingAPassedResultAgainFails) {
  ...
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(result, matcher),
      "TestResult for Required.YieldsTest.foo.ProtobufInput.ProtobufOutput was "
...
TEST_F(YieldsTest, JsonOutput) {
  ...
  auto run_json = [&](absl::string_view response, absl::string_view name) {
    runner_.RespondWith(response);
    return testee_.CreateTest(Named(name), TestStrictness::kRequired)
  ...
  EXPECT_THAT(
      test_manager_.UnexpectedFailures(),
      ElementsAre(
          FieldsAre("Required.YieldsTest.invalid.JsonInput.JsonOutput",
                    testing::StartsWith(
                        "JSON payload cannot be parsed as valid JSON: "),
                    absl::nullopt),
          FieldsAre("Required.YieldsTest.mismatch.JsonInput.JsonOutput",
                    "Expect: JSON payload has member \"optionalInt64\", but "
                    "got: \"{\\\"optionalInt32\\\": 9}\"",
                    absl::nullopt),
          FieldsAre("Required.YieldsTest.unparseable.JsonInput.JsonOutput",
                    "JSON output we received from test was unparseable.",
                    absl::nullopt)));
}
