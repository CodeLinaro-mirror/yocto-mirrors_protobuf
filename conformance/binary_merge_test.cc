...
// TestOverwriteMessageValueMap() and TestMergeOneofMessage(); the requests sent
...
TEST_P(MergeTest, MapMessageValue) {
  ...
  EXPECT_THAT(
      RequiredTest().ParseBinary(message(), data.input).SerializeBinary(),
      Yields(ParsedPayload(EqualsBinaryProto(data.expected))));
}

TEST_P(MergeTest, MapMessageValueJson) {
  MergeTestData data = MapMessageValueMergeData(*message());
  EXPECT_THAT(RequiredTest().ParseBinary(message(), data.input).SerializeJson(),
              Yields(ParsedPayload(EqualsBinaryProto(data.expected))));
}
...
TEST_P(MergeTest, OneofMessageBinary) {
  MergeTestData data = OneofMessageMergeData(*message());
  EXPECT_THAT(
      RecommendedTest().ParseBinary(message(), data.input).SerializeBinary(),
