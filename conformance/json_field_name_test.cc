...
// RunJsonTestsForStoresDefaultPrimitive(); the requests sent to the testee are
...
TEST_P(JsonFieldNameTest, FieldNameWithDoubleUnderscores) {
  ...
  EXPECT_THAT(RecommendedTest().ParseJson(message(), kInput).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(RecommendedTest().ParseJson(message(), kInput).SerializeJson(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonFieldNameTest, FieldNameNotQuoted) {
  EXPECT_THAT(
      RecommendedTest().ParseJson(message(), "{fieldname1: 1}").ParseOnly(),
      Yields(IsParseError()));
}
