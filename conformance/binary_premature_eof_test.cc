...
// RunAllTests() called for every field type except groups; the requests sent to
...
INSTANTIATE_TEST_SUITE_P(All, PrematureEofTest,
                         Combine(ValuesIn(AllTestMessageTypes()),
                                 ValuesIn(AllFieldTypesExceptGroup())),
...
TEST_P(PrematureEofInDelimitedDataTest, ForUnknownValue) {
  EXPECT_THAT(RequiredTest()
                  .ParseBinary(message(), Wire(Tag(kUnknownFieldNumber,
                                                   WireType::kLengthPrefixed),
                                               Varint(1)))
                  .ParseOnly(),
              Yields(IsParseError()));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Combine(ValuesIn(AllTestMessageTypes()), ValuesIn(PackableFieldTypes())),
...
// message type alone; the test's name, Message, keeps the legacy ".MESSAGE"
// suffix.
