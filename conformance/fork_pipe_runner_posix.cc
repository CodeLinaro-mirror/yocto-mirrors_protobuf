  ...
  while (len > 0) {
    ...
    if (status == std::future_status::timeout) {
      ...
      *timed_out = true;
      ...
      do {
        err_bytes_read =
            read(state_->read_fd, (void*)&err[err_ofs], err.size() - err_ofs);
        if (err_bytes_read > 0) {
      ...
      ABSL_LOG(ERROR) << "child_pid_=" << state_->child_pid << " SIGQUIT: \n"
                      << absl::string_view(err.data(), err_ofs);
      // Some test programs survive SIGQUIT (e.g. a JVM just prints a thread
      // dump), which would make the blocking waitpid() in
      // GetTestProgramFailure() hang forever. Make sure the child is dead.
      kill(state_->child_pid, SIGKILL);
      return false;
    }
