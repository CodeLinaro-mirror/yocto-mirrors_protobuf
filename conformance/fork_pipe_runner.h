...
// until the parent process exits.  Once the testee has exited, whatever it
// left behind in its process group (POSIX) or job object (Windows) is killed
// too.
//
...
class ForkPipeRunner : public ConformanceTestRunner {
 ...
 private:
  ...
  struct ShutdownResult {
    ...
    // killed it (in which case `wait_status` normally says so too).  Not set
    // by the sweep of what the testee left behind after it exited on its own.
    bool killed = false;
  };
  ...
  // testee is reaped; on Windows closing the job object, which is configured
  // to kill its remaining processes on close, does the same.  Safe to call
  // more than once and when no testee was ever spawned.
  ShutdownResult Shutdown(std::chrono::milliseconds grace_period);

  // Writes all `len` bytes of `buf` to the testee.  Returns false if the
  // write failed, in particular because the testee has exited (EPIPE on
  // POSIX, where SIGPIPE is blocked on the calling thread for the duration of
  // the write and the one it raises is discarded; a broken-pipe error on
  // Windows), so a testee that dies between two requests is reported instead
  // of killing the runner.  The caller is expected to ReportFailure() on
  // failure.
  ...
  // blocks indefinitely; the caller is expected to ReportReadFailure() on
  // failure.
  ...
  // `timed_out` the report is a timeout_error and the testee, which on POSIX
  // has just been sent SIGQUIT, is given a moment to die of that before it is
  // killed; otherwise the report is a runtime_error and the testee, which has
  // normally exited already, gets a shorter grace period (see
  // kExitGracePeriod in the implementation).  `fallback_error` is reported
  // when there was no wait status to classify.  The next RunTest() call spawns
  // a fresh testee.
