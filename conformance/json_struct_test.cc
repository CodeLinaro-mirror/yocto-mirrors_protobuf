...
TEST_P(JsonStructTest, Struct) {
  ...
  EXPECT_THAT(
      RequiredTest("Struct").ParseJson(message(), kInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(
      RequiredTest("Struct").ParseJson(message(), kInput).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonStructTest, StructWithEmptyListValue) {
  ...
  EXPECT_THAT(RequiredTest("StructWithEmptyListValue")
  ...
  const std::string expected =
      absl::StrCat("optional_struct: {\n",
                   ...
                   Repeat("      }\n"
                          ...
                          25),
                   "}\n");
  EXPECT_THAT(RecommendedTest("StructDeepNesting25")
...
TEST_P(JsonStructTest, StructDeepNesting200) {
  ...
  EXPECT_THAT(RecommendedTest("StructDeepNesting200")
                  .ParseJson(message(), input)
                  .ParseOnly(),
              Yields(IsParseError()));
}
