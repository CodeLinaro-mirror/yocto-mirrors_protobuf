...
// and its helpers; the requests sent to the testee are identical to the legacy
// ones.
...
using UnknownFieldsPerformanceTest = MessageTypeConformanceTest;
...
TEST_P(UnknownFieldsPerformanceTest, AlternatingUnknownFields) {
  ...
  EXPECT_THAT(RecommendedTest().ParseBinary(message(), input).SerializeBinary(),
