...
def _subject_of(target):
    ...
    return target.label.name.removesuffix("_binary_test").removesuffix("_performance_test")

def _failure_list_path(target, suite = "binary"):
    """The workspace-relative path of `target`'s failure list for `suite`."""
    return "%s/%s/%s_%s.txt" % (target.label.package, _FAILURE_LISTS, _subject_of(target), suite)
...
def _test_with_failure_list_impl(env, targets):
    ...
    performance_failure_list = _failure_list_path(targets.performance_test, "performance")
