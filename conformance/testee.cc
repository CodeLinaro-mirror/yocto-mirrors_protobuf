...
std::string GetTestName(const TestName& name, TestStrictness strictness,
                        const ::conformance::ConformanceRequest& request,
                        NameStyle name_style) {
  std::string full_name =
      absl::StrCat(StrictnessName(strictness), ".", name.suite, ".", name.test);
...
InMemoryMessage Test::ParseBinary(const Descriptor* type, Wire input) && {
  ...
  return InMemoryMessage(testee_, std::move(name_), strictness_, type,
                         std::move(request));
}
...
TestResult InMemoryMessage::Finish(
  ...
  std::string full_name = GetTestName(name_, strictness_, request_, name_style);
