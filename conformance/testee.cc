...
// ".<Output>Output" unless `name_style` omits it.  A legacy explicit name is
// used as "<Level>.<Syntax>.<Input>Input.<legacy_name>" instead.
std::string GetTestName(const TestName& name, absl::string_view legacy_name,
                        TestStrictness strictness,
                        const ::conformance::ConformanceRequest& request,
                        const Descriptor& message, NameStyle name_style) {
  const std::string input =
      absl::StrCat(GetFormatIdentifier(GetInputFormat(request)), "Input");
  std::string full_name(StrictnessName(strictness));
  if (!legacy_name.empty()) {
    absl::StrAppend(&full_name, ".", GetEditionIdentifier(message), ".", input,
                    ".", legacy_name);
  } else {
    absl::StrAppend(&full_name, ".", name.suite, ".", name.test);
    if (!name.params.empty()) absl::StrAppend(&full_name, "/", name.params);
    if (!name.suffix.empty()) absl::StrAppend(&full_name, ".", name.suffix);
    absl::StrAppend(&full_name, ".", input);
  }
...
Test Testee::CreateTest(TestName name, TestStrictness strictness) {
  ...
  return Test(this, std::move(name), "", strictness);
}
...
InMemoryMessage Test::ParseBinary(const Descriptor* type, Wire input) && {
  ...
  return InMemoryMessage(testee_, name_, legacy_name_, strictness_, type,
                         std::move(request));
}
...
TestResult InMemoryMessage::Finish(
  ...
  std::string full_name = GetTestName(name_, legacy_name_, strictness_,
                                      request_, *type_, name_style);
