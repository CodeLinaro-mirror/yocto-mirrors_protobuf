...
// "<Strictness>.<Edition>.<InputFormat>Input.<name>", followed by
// ".<OutputFormat>Output" unless the style omits it.
std::string GetTestName(absl::string_view test_name, TestStrictness strictness,
  ...
  std::string full_name = absl::StrCat(
      StrictnessName(strictness), ".", GetEditionIdentifier(message), ".",
...
InMemoryMessage&& InMemoryMessage::OverrideTestCategory(
  ...
  ABSL_DCHECK_NE(category, ::conformance::JSON_IGNORE_UNKNOWN_PARSING_TEST)
      << "OverrideTestCategory() only relabels tests; use "
