...
// (REQUIRED, equivalence) and ValidDataOneofBinary.<TYPE>.<Case>
// (RECOMMENDED, byte-exact) for the cases DefaultValue, NonDefaultValue,
// MultipleValuesForSameField and MultipleValuesForDifferentField.
//
// This holds the binary-output leg of the legacy
// BinaryAndJsonConformanceSuiteImpl<M>::TestValidDataForOneofType(); the test
// names and the requests sent to the testee are identical to the legacy ones,
// and the legacy suite iterates the same type table for the binary->JSON legs
// it still sends.  The oneof merge and zero-value tests live in
// binary_merge_test.cc and binary_oneof_zero_test.cc.
//
...
class ValidDataOneofTest
 ...
 protected:
  ...
  // "ValidDataOneof.UINT32.DefaultValue".
...
TEST_P(ValidDataOneofTest, DefaultValue) {
  EXPECT_THAT(RequiredTest(TestName("ValidDataOneof", "DefaultValue"))
                  .ParseBinary(message(), DefaultMember())
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(DefaultMember()))));
}

// ... and its serialization must be exactly the input.
TEST_P(ValidDataOneofTest, DefaultValueBinary) {
  EXPECT_THAT(RecommendedTest(TestName("ValidDataOneofBinary", "DefaultValue"))
                  .ParseBinary(message(), DefaultMember())
                  .SerializeBinary(),
...
TEST_P(ValidDataOneofTest, NonDefaultValue) {
  EXPECT_THAT(RequiredTest(TestName("ValidDataOneof", "NonDefaultValue"))
                  .ParseBinary(message(), NonDefaultMember())
                  .SerializeBinary(),
...
TEST_P(ValidDataOneofTest, NonDefaultValueBinary) {
  EXPECT_THAT(
      RecommendedTest(TestName("ValidDataOneofBinary", "NonDefaultValue"))
          .ParseBinary(message(), NonDefaultMember())
          .SerializeBinary(),
...
TEST_P(ValidDataOneofTest, MultipleValuesForSameField) {
  EXPECT_THAT(
      RequiredTest(TestName("ValidDataOneof", "MultipleValuesForSameField"))
          .ParseBinary(message(), Wire(DefaultMember(), NonDefaultMember()))
          .SerializeBinary(),
      Yields(ParsedPayload(EqualsBinaryProto(NonDefaultMember()))));
}

// ... and the serialization must be exactly the last value.
TEST_P(ValidDataOneofTest, MultipleValuesForSameFieldBinary) {
  EXPECT_THAT(
      RecommendedTest(
          TestName("ValidDataOneofBinary", "MultipleValuesForSameField"))
          .ParseBinary(message(), Wire(DefaultMember(), NonDefaultMember()))
          .SerializeBinary(),
      Yields(Payload(NonDefaultMember())));
}
...
TEST_P(ValidDataOneofTest, MultipleValuesForDifferentField) {
  EXPECT_THAT(
      RequiredTest(
          TestName("ValidDataOneof", "MultipleValuesForDifferentField"))
          .ParseBinary(message(), Wire(OtherMemberValue(), NonDefaultMember()))
          .SerializeBinary(),
      Yields(ParsedPayload(EqualsBinaryProto(NonDefaultMember()))));
}

// ... and the serialization must be exactly the last member.
TEST_P(ValidDataOneofTest, MultipleValuesForDifferentFieldBinary) {
  EXPECT_THAT(
      RecommendedTest(
          TestName("ValidDataOneofBinary", "MultipleValuesForDifferentField"))
          .ParseBinary(message(), Wire(OtherMemberValue(), NonDefaultMember()))
          .SerializeBinary(),
      Yields(Payload(NonDefaultMember())));
}
