...
// INSTANTIATE_TEST_SUITE_P prefix ("All/FooTest" -> "FooTest"), its name
// split at the "/" that precedes a parameter name ("Bar/Proto3_INT32"), and
// `suffix`.
internal::TestName CurrentTestName(absl::string_view suffix) {
  const testing::TestInfo* test_info =
      testing::UnitTest::GetInstance()->current_test_info();
  ABSL_CHECK(test_info != nullptr)
      << "RequiredTest() and RecommendedTest() can only be called from inside "
         "a test body.";
...
internal::Test RequiredTest(absl::string_view suffix) {
  return ConformanceEnvironment::Get().testee().CreateTest(
      CurrentTestName(suffix), internal::TestStrictness::kRequired);
}

internal::Test RecommendedTest(absl::string_view suffix) {
  return ConformanceEnvironment::Get().testee().CreateTest(
      CurrentTestName(suffix), internal::TestStrictness::kRecommended);
}
