...
#include <limits.h>  // for PATH_MAX, which <climits> doesn't guarantee
#include <unistd.h>
...
// The TestName of the running gtest test: its suite without the
// INSTANTIATE_TEST_SUITE_P prefix ("All/FooTest" -> "FooTest") and its name
// split at the "/" that precedes a parameter name ("Bar/Proto3_INT32").
internal::TestName CurrentTestName() {
  const testing::TestInfo* test_info =
      testing::UnitTest::GetInstance()->current_test_info();
  ABSL_CHECK(test_info != nullptr)
      << "RequiredTest() and RecommendedTest() can only infer the test name "
         "from inside a test body; pass a name explicitly otherwise.";
  ABSL_CHECK(test_info->type_param() == nullptr)
      << "Typed and type-parameterized tests are not supported: their suite "
         "names are not stable identifiers.";
  internal::TestName name;
  absl::string_view suite = test_info->test_suite_name();
  // For a suite without a prefix, find() is npos and npos + 1 wraps to 0.
  name.suite = std::string(suite.substr(suite.find('/') + 1));
  std::pair<absl::string_view, absl::string_view> test_and_params =
      absl::StrSplit(test_info->name(), absl::MaxSplits('/', 1));
  name.test = std::string(test_and_params.first);
  name.params = std::string(test_and_params.second);
  return name;
}
...
ConformanceEnvironment::ConformanceEnvironment(
  ...
  // Anything below proto3 (including conformance_test_runner's EDITION_UNKNOWN
...
ConformanceEnvironment& ConformanceEnvironment::Install(
  ...
  // the merged conformance_test_runner still needs the test manager
  // afterwards.  So although this class is a testing::Environment, it isn't
  // registered with gtest itself: gtest only gets a forwarding proxy, and the
  // real environment intentionally lives until the process exits.  Only the
  // environment object leaks, though; TearDown() releases the testee.
  // TODO: b/563707827 - register it directly once conformance_test_main.cc no
  // longer needs it after RUN_ALL_TESTS() (see the class comment).
...
void ConformanceEnvironment::TearDown() {
  ...
  if (options_.fix && set_up_succeeded_) {
    if (partial_run) {
      ...
      if (output_file.empty()) {
        if (options_.failure_list_files.empty()) {
          ...
          ADD_FAILURE() << "--fix needs --fix_output_file when more than one "
                           "--failure_list is given.";
        } else if (absl::StartsWith(options_.failure_list_files[0], "/")) {
      ...
      } else if (!absl::StartsWith(output_file, "/") && workspace != nullptr) {
      ...
      if (!output_file.empty()) {
        const bool relative = !absl::StartsWith(output_file, "/");
  ...
  if (!status.ok() && set_up_succeeded_ &&
    ...
    if (partial_run) {
      ...
      // exercised.  The conformance_test_runner --test flag behaves the same
      // way.
...
bool ConformanceTest::IsSupported(const Descriptor* absl_nonnull descriptor) {
  ...
  if (edition == EDITION_UNSTABLE) {
    // `maximum_edition` is the maximum *stable* edition; EDITION_UNSTABLE is
    // newer than any of them, so it is gated on the editions tier instead: the
    // unstable tests run whenever the 2023 tests do, as in the original suites
    // and the open-source runner.  Every test message the suites use is 2023
    // or older, so nothing is skipped that the unstable tests would leapfrog.
    // TODO: b/563659302 - once a test message of a newer stable edition
    // exists, give EDITION_UNSTABLE its own opt-in (an `unstable_edition`
    // option / flag / conformance_test() attribute) so that
    // --maximum_edition=2023 keeps skipping the newer stable message while
    // still running these.
...
internal::Test RequiredTest() {
  return ConformanceEnvironment::Get().testee().CreateTest(
      CurrentTestName(), internal::TestStrictness::kRequired);
}

internal::Test RequiredTest(absl::string_view name) {
  return ConformanceEnvironment::Get().testee().CreateTest(
      name, internal::TestStrictness::kRequired);
}

internal::Test RecommendedTest() {
  return ConformanceEnvironment::Get().testee().CreateTest(
      CurrentTestName(), internal::TestStrictness::kRecommended);
}

internal::Test RecommendedTest(absl::string_view name) {
  return ConformanceEnvironment::Get().testee().CreateTest(
      name, internal::TestStrictness::kRecommended);
}
