...
// one.  This replaces the singular-field part of the legacy
// BinaryAndJsonConformanceSuiteImpl<M>::TestValidDataForType(); the requests
...
// expecting the same canonical message.
...
// ... also when serialized as JSON ...
TEST_P(ValidDataScalarTest, ScalarJson) {
  EXPECT_THAT(RequiredTest()
                  .ParseBinary(message(), FieldWithValue(value().input))
                  .SerializeJson(),
              Yields(ParsedPayload(EqualsBinaryProto(Expected()))));
...
// ... and its binary serialization must be exactly the canonical form.
...
TEST_P(RepeatedScalarSelectsLastTest, SelectsLast) {
  ...
  EXPECT_THAT(RequiredTest().ParseBinary(message(), input).SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(
                  FieldWithValue(ValidDataCases(type()).back().expected)))));
}

TEST_P(RepeatedScalarSelectsLastTest, SelectsLastJson) {
  Wire input;
  for (const ValidDataCase& value : ValidDataCases(type())) {
    input = Wire(input, FieldWithValue(value.input));
  }
  EXPECT_THAT(RequiredTest().ParseBinary(message(), input).SerializeJson(),
              Yields(ParsedPayload(EqualsBinaryProto(
                  FieldWithValue(ValidDataCases(type()).back().expected)))));
}
