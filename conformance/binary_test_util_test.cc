...
TEST(ValidDataCasesTest, LegacyTableSizes) {
  const std::map<FieldDescriptor::Type, size_t> kSizes = {
      ...
      {FieldDescriptor::TYPE_SINT64, 4},   {FieldDescriptor::TYPE_STRING, 7},
...
TEST(ValidDataCasesTest, NonCanonicalInputs) {
  ...
  // A unicode escape and the literal UTF-8 it denotes.
  EXPECT_EQ(ValidDataCases(FieldDescriptor::TYPE_STRING)[4].input,
            LengthPrefixed("\xE8\xB0\xB7\xE6\xAD\x8C"));
  EXPECT_EQ(ValidDataCases(FieldDescriptor::TYPE_STRING)[4].expected,
            LengthPrefixed("谷歌"));
}
...
TEST(ParamNameTest, ResultsAreValidGtestNameComponents) {
  ...
  for (const MapType& type : ValidDataMapTypes()) {
    EXPECT_THAT(ParamName(type), MatchesRegex("[A-Za-z0-9]+_[A-Za-z0-9]+"));
  }
}
