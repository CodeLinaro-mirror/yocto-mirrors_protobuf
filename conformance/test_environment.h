...
struct ConformanceEnvironmentOptions {
  ...
  // wildcard entries included, are TestManager::SaveFailureList()'s).  Refused
  // (with a test failure, and without writing anything) when only a subset of
  // the binary's tests ran (see internal::IsPartialRun()), since the entries
  // of the tests that didn't run would otherwise be dropped as "unseen".
  bool fix = false;
