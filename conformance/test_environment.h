...
//     handles edition gating and per-test statistics.
//   - RequiredTest()/RecommendedTest(): entry points for building a test
//     against the global testee.
//
// Example:
//
...
struct ConformanceEnvironmentOptions {
  ...
  // test would.  The default matches the conformance_test_runner command line.
  ...
  // whenever this is EDITION_2023 or newer (as with the conformance_test_runner
  // command line).  Values below EDITION_PROTO3 (e.g. EDITION_UNKNOWN, the
  // conformance_test_runner default) are meaningless, since proto2 and proto3
  // tests always run; the environment clamps them to EDITION_PROTO3 in its
  // constructor, and that is the one place the clamp lives.  The default only
  // runs proto2 and proto3 tests, like the conformance_test_runner's.
  Edition maximum_edition = EDITION_PROTO3;

  // Rewrite the failure list at the end of the run, dropping entries that no
  // longer fail and adding new failures with their messages (the rules,
  // wildcard entries and copybara strip blocks included, are
  // TestManager::SaveFailureList()'s).  Refused (with a test failure, and
  // without writing anything) when only a subset of the binary's tests ran
  // (see internal::IsPartialRun()), since the entries of the tests that didn't
  // run would otherwise be dropped as "unseen".
  bool fix = false;
  ...
  // to that single file.  Relative paths (this one's or the failure list's)
  // are resolved under $BUILD_WORKSPACE_DIRECTORY when it is set, i.e. to the
  // source file when run via `bazel run`.
  ...
  // a partial run can't see every expected failure; the conformance_test_runner
  // --test flag behaves the same way.  The merged runner
  // (conformance_test_main.cc) sets this to false and reports the unmatched
  // entries itself, per failure list flag and in the conformance_test_runner
  // report format (see TestManager::UnmatchedExpectedFailures).
...
// runner (conformance_test_main.cc) reads test_manager() afterwards to print
// the results in the conformance_test_runner report format.  Install() instead
...
// The fixture for all conformance tests.
//
...
// performance suite is a library of its own, so its own test binaries need no
// such rule, but conformance_test_runner links every suite into one binary
// and selects the performance tests for its --performance flag by that suffix
// (see PerformanceGtestFilter() in conformance_test_runner.h).  Nothing checks
// the rule mechanically: a misnamed fixture silently runs with the wrong flag.
...
// Creates a required test against the global testee, named after the current
// gtest test (see internal::TestName):
//
//   <Level>.<Suite>.<Test>[/<Params>][.<Suffix>].<Input>Input[.<Output>Output]
//
// e.g. "Required.PrematureEofTest.BeforeKnownNonRepeatedValue/Proto3_INT32.
// ProtobufInput" for the (proto3, INT32) instance of
// TEST_P(PrematureEofTest, BeforeKnownNonRepeatedValue) parameterized over
// (message type, field type).  Only plain and value-parameterized tests are
// supported (typed tests check-fail); the INSTANTIATE_TEST_SUITE_P prefix is
// dropped.  Must be called from inside a test body.
//
// A test body that sends more than one request with the same input and output
// formats gives each its own `suffix` (e.g. RecommendedTest("Drop") and
// RecommendedTest("Print")); a duplicated full name is fatal.  The suffix
// becomes one segment of the name, so it may only contain [A-Za-z0-9_].
internal::Test RequiredTest(absl::string_view suffix = "");

// Creates a recommended test against the global testee.  See RequiredTest().
internal::Test RecommendedTest(absl::string_view suffix = "");
