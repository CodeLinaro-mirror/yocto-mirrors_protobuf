...
// BinaryAndJsonConformanceSuiteImpl<M>::TestUnknownWireType(); the requests
...
TEST_P(UnknownWireTypeTest, Rejected) {
  ...
  EXPECT_THAT(RequiredTest().ParseBinary(message(), Wire(payload)).ParseOnly(),
              Yields(IsParseError()));
}

// Instance names are "<Message>_WireType<n>_Field<n>_Value<n>", so the
// conformance tests are named e.g. "Required.UnknownWireTypeTest.Rejected/
// Proto3_WireType6_Field0_Value0.ProtobufInput".
INSTANTIATE_TEST_SUITE_P(
    All, UnknownWireTypeTest,
    Combine(ValuesIn(AllTestMessageTypes()), Values(6, 7), Range(0, 4),
            Range(0, 4)),
    [](const testing::TestParamInfo<UnknownWireTypeTest::ParamType>& info) {
      return absl::StrCat(ParamName(std::get<0>(info.param)), "_WireType",
                          std::get<1>(info.param), "_Field",
                          std::get<2>(info.param), "_Value",
