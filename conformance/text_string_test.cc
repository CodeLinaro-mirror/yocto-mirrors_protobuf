...
TEST_P(TextStringLiteralTest, ShortUnicodeEscape) {
  ...
  EXPECT_THAT(RecommendedTest().ParseText(message(), input).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RecommendedTest().ParseText(message(), input).SerializeText(),
              Yields(ParsedPayload(EqualsTextProto(input))));
}
...
TEST_P(TextStringLiteralTest, LongUnicodeEscapeTooLarge) {
  EXPECT_THAT(
      RequiredTest().ParseText(message(), Input("'\\U00110000'")).ParseOnly(),
      Yields(IsParseError()));
}
...
TEST_P(TextStringLiteralTest, ShortUnicodeEscapeSurrogateFirstOnly) {
  EXPECT_THAT(
      RecommendedTest().ParseText(message(), Input("'\\ud800'")).ParseOnly(),
      Yields(IsParseError()));
}
