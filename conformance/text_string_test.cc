...
class TextStringLiteralTest : public ConformanceTest,
 ...
 protected:
  ...
  // The legacy test name: "StringLiteral<base><String|Bytes>", e.g.
  // "StringLiteralConcatBytes".
  std::string TestName(absl::string_view base) const {
    return absl::StrCat("StringLiteral", base, field().type_name);
  }
...
TEST_P(TextStringLiteralTest, Concat) {
  ...
  EXPECT_THAT(RequiredTest(TestName("Concat"))
...
TEST_P(TextStringLiteralTest, BasicEscapes) {
  ...
  EXPECT_THAT(RequiredTest(TestName("BasicEscapes"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RequiredTest(TestName("BasicEscapes"))
...
TEST_P(TextStringLiteralTest, OctalEscapes) {
  ...
  EXPECT_THAT(RequiredTest(TestName("OctalEscapes"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RequiredTest(TestName("OctalEscapes"))
...
TEST_P(TextStringLiteralTest, HexEscapes) {
  ...
  EXPECT_THAT(RequiredTest(TestName("HexEscapes"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RequiredTest(TestName("HexEscapes"))
...
TEST_P(TextStringLiteralTest, ShortUnicodeEscape) {
  ...
  EXPECT_THAT(RecommendedTest(TestName("ShortUnicodeEscape"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RecommendedTest(TestName("ShortUnicodeEscape"))
...
TEST_P(TextStringLiteralTest, LongUnicodeEscapes) {
  ...
  EXPECT_THAT(RecommendedTest(TestName("LongUnicodeEscapes"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(RecommendedTest(TestName("LongUnicodeEscapes"))
...
TEST_P(TextStringLiteralTest, IncludesLF) {
  EXPECT_THAT(RequiredTest(TestName("IncludesLF"))
...
TEST_P(TextStringLiteralTest, LongUnicodeEscapeTooLarge) {
  EXPECT_THAT(RequiredTest(TestName("LongUnicodeEscapeTooLarge"))
                  .ParseText(message(), Input("'\\U00110000'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextStringLiteralTest, ShortUnicodeEscapeSurrogatePair) {
  EXPECT_THAT(RecommendedTest(TestName("ShortUnicodeEscapeSurrogatePair"))
                  .ParseText(message(), Input("'\\ud801\\udc37'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextStringLiteralTest, ShortUnicodeEscapeSurrogateFirstOnly) {
  EXPECT_THAT(RecommendedTest(TestName("ShortUnicodeEscapeSurrogateFirstOnly"))
                  .ParseText(message(), Input("'\\ud800'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextStringLiteralTest, ShortUnicodeEscapeSurrogateSecondOnly) {
  EXPECT_THAT(RecommendedTest(TestName("ShortUnicodeEscapeSurrogateSecondOnly"))
                  .ParseText(message(), Input("'\\udc00'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextStringLiteralTest, LongUnicodeEscapeSurrogateFirstOnly) {
  EXPECT_THAT(RecommendedTest(TestName("LongUnicodeEscapeSurrogateFirstOnly"))
                  .ParseText(message(), Input("'\\U0000d800'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(TextStringLiteralTest, LongUnicodeEscapeSurrogateSecondOnly) {
  EXPECT_THAT(RecommendedTest(TestName("LongUnicodeEscapeSurrogateSecondOnly"))
                  .ParseText(message(), Input("'\\U0000dc00'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(TextStringLiteralTest, LongUnicodeEscapeSurrogatePair) {
  EXPECT_THAT(RecommendedTest(TestName("LongUnicodeEscapeSurrogatePair"))
...
TEST_P(TextStringLiteralTest, UnicodeEscapeSurrogatePairLongShort) {
  EXPECT_THAT(RecommendedTest(TestName("UnicodeEscapeSurrogatePairLongShort"))
                  .ParseText(message(), Input("'\\U0000d801\\udc37'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(TextStringLiteralTest, UnicodeEscapeSurrogatePairShortLong) {
  EXPECT_THAT(RecommendedTest(TestName("UnicodeEscapeSurrogatePairShortLong"))
                  .ParseText(message(), Input("'\\ud801\\U0000dc37'"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextStringFieldTest, BadUtf8Octal) {
  EXPECT_THAT(RequiredTest("StringFieldBadUTF8Octal")
                  .ParseText(message(), "optional_string: '\\300'")
                  .ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(TextStringFieldTest, BadUtf8Hex) {
  EXPECT_THAT(RequiredTest("StringFieldBadUTF8Hex")
                  .ParseText(message(), "optional_string: '\\xc0'")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextBytesFieldTest, BadUtf8Octal) {
  EXPECT_THAT(
      RequiredTest("BytesFieldBadUTF8Octal")
...
TEST_P(TextBytesFieldTest, BadUtf8Hex) {
  EXPECT_THAT(
      RequiredTest("BytesFieldBadUTF8Hex")
