...
// value (zero, empty, or an empty message) survives a round trip: unlike a
// plain proto3 field, a oneof member always has presence.  This holds the
// binary-output leg of the legacy
...
TEST_P(OneofZeroTest, MessageSetTwice) {
  EXPECT_THAT(
      RecommendedTest("OneofZeroMessageSetTwice")
...
TEST_P(OneofZeroTest, String) {
  EXPECT_THAT(
      RecommendedTest("OneofZeroString")
