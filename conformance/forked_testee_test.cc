...
TEST(ForkedTesteeTest, AcceptsANonexistentBinary) {
  // MakeTesteeRunner() only builds the runner; ForkPipeRunner spawns the
  // testee on the first request, so the binary need not even exist until a
  // test runs (--gtest_list_tests and an all-skipped run never do).
  std::unique_ptr<ConformanceTestRunner> runner =
