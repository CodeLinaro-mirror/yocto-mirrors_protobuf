...
class TestManager {
 public:
  ...
  // inserted in name order with their messages.  Returns an error if
  // `filename` can't be opened for writing or the write fails.
  ...
  // entry is kept, also by SaveFailureList() under --fix (as is, or as the
  // test's own line when a wildcard entry is expanded), so removing the entry
  // is up to the user.
