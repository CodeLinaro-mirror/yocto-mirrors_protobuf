...
class TestManager {
 public:
  ...
  // entry whose matched tests all still fail with its message is kept
  // verbatim, wildcards included.  An entry whose matched tests all failed
  // with the same different message is kept with that message instead,
  // wildcards included.  Any other entry whose matches changed, i.e. a test it
  // matched succeeded or was skipped (see ReportSkip()) or the matched tests
  // failed with differing messages, is rewritten in place, one line per
  // matched test in name order: a test that succeeded or was skipped is left
  // out, a test that failed with a different message gets that message, and a
  // test that still fails with the entry's message keeps it.  For an exact
  // entry that means the line goes when its test succeeded or was skipped; for
  // a wildcard entry it means the wildcard is expanded into the tests it
  // matched, so that the tests that still fail stay listed.  Tests that failed
  // but matched no entry are inserted in name order with their messages.
  ...
  // Reports a test that was skipped by the testee for `skip_reason`.  A
  // skipped test can't be an expected failure, so if it matched a failure list
  // entry the entry counts as seen and matched (so that Finalize() doesn't
  // report it as unseen) but the match is dropped by SaveFailureList(), like a
  // success.  A listed test that is skipped is recorded (see ListedSkips())
  // and, like an unexpected success, returns an error that names the matched
  // entry.  Whether that error fails the test is the caller's policy (the
  // matchers fail such a test).
  ...
  // kTestNotSelectedSkipReason in test_runner.h).
  ...
  // ReportSkip() returns), but it is not an unexpected failure or success:
  // SaveFailureList() under --fix drops its match like a passing test's (the
  // whole entry, or the test's own line when a wildcard entry is expanded).
