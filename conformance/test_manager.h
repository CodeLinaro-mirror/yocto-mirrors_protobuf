...
class TestManager {
 public:
  ...
  // test name is already present (or overlaps a wildcard entry).
  ...
  // matchers in matchers.h fail such a test).
  ...
  // success: SaveFailureList() under --fix drops its match like a passing
  // test's (the whole entry, or the test's own line when a wildcard entry is
  // expanded).
