...
// TestMapEntryWireTypeMismatch(); the test names and the requests sent to the
// testee are identical to the legacy ones.  Like the legacy
// RunValidProtobufTest(), each ValidDataMap input is sent twice: once to be
// serialized as binary ("<name>.ProtobufOutput") and once as JSON
// ("<name>.JsonOutput"), and both outputs must be equivalent to the same
// reference message.  The wire-type mismatch tests were binary-only.
