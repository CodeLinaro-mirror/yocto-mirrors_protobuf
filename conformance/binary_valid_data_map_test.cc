...
// BinaryAndJsonConformanceSuiteImpl<M>::TestValidDataForMapType() and all of
// the legacy TestMapEntryWireTypeMismatch(); the test names and the requests
// sent to the testee are identical to the legacy ones, and the legacy suite
// iterates the same type table for the binary->JSON legs it still sends.
//
...
class MapTestBase : public ConformanceTest,
 ...
 protected:
  ...
  // The legacy test name: "ValidDataMap.<KEY>.<VALUE>.<suffix>", e.g.
  // "ValidDataMap.INT32.INT32.Default".
  std::string TestName(absl::string_view suffix) const {
    return absl::StrCat("ValidDataMap.", UpperCaseTypeName(key_type()), ".",
                        UpperCaseTypeName(value_type()), ".", suffix);
  }
...
TEST_P(ValidDataMapTest, Default) {
  ...
  EXPECT_THAT(RequiredTest(TestName("Default"))
...
TEST_P(ValidDataMapTest, MissingDefault) {
  ...
  EXPECT_THAT(RequiredTest(TestName("MissingDefault"))
                  .ParseBinary(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(input))));
}
...
TEST_P(ValidDataMapTest, NonDefault) {
  ...
  EXPECT_THAT(RequiredTest(TestName("NonDefault"))
                  .ParseBinary(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(input))));
}
...
TEST_P(ValidDataMapTest, Unordered) {
  ...
  EXPECT_THAT(RequiredTest(TestName("Unordered"))
                  .ParseBinary(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(input))));
}
...
TEST_P(ValidDataMapTest, DuplicateKey) {
  ...
  const Wire second =
      MapEntry(Wire(NonDefaultKeyField(), NonDefaultValueField()));
  EXPECT_THAT(RequiredTest(TestName("DuplicateKey"))
                  .ParseBinary(message(), Wire(first, second))
                  .SerializeBinary(),
...
TEST_P(ValidDataMapTest, DuplicateKeyInMapEntry) {
  ...
  EXPECT_THAT(RequiredTest(TestName("DuplicateKeyInMapEntry"))
                  .ParseBinary(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(input))));
}
...
TEST_P(ValidDataMapTest, DuplicateValueInMapEntry) {
  ...
  EXPECT_THAT(RequiredTest(TestName("DuplicateValueInMapEntry"))
                  .ParseBinary(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsBinaryProto(input))));
}
...
TEST_P(MapEntryWireTypeMismatchTest, KeyWireTypeMismatch) {
  ...
  EXPECT_THAT(
      RecommendedTest(TestName("KeyWireTypeMismatch"))
          .ParseBinary(message(), input)
          .SerializeBinary(),
...
TEST_P(MapEntryWireTypeMismatchTest, ValueWireTypeMismatch) {
  ...
  EXPECT_THAT(
      RecommendedTest(TestName("ValueWireTypeMismatch"))
          .ParseBinary(message(), input)
          .SerializeBinary(),
      Yields(AnyOf(IsParseError(), ParsedPayload(EqualsBinaryProto(input)))));
}
