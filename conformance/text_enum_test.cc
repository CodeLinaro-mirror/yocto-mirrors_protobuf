...
// RunOpenEnumTests() and RunClosedEnumTests(); the requests sent to the testee
...
TEST_P(TextOpenEnumTest, ByNumber) {
  EXPECT_THAT(
      RequiredTest().ParseText(message(), kKnownNumberInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kKnownNumberInput))));
  EXPECT_THAT(
      RequiredTest().ParseText(message(), kKnownNumberInput).SerializeText(),
      Yields(ParsedPayload(EqualsTextProto(kKnownNumberInput))));
}
...
TEST_P(TextOpenEnumTest, WithUnknownNumber) {
  ...
  EXPECT_THAT(
      RequiredTest().ParseText(message(), kUnknownNumberInput).SerializeText(),
      Yields(ParsedPayload(EqualsTextProto(kUnknownNumberInput))));
}
...
TEST_P(TextClosedEnumTest, WithUnknownNumber) {
  EXPECT_THAT(
      RequiredTest().ParseText(message(), kUnknownNumberInput).ParseOnly(),
      Yields(IsParseError()));
}
